...
class RegisterViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let gesture = UITapGestureRecognizer(target: self, action:
                                                #selector(didTapChangeProfilePic))
    ...
    @objc private func RegisterButtonTapped() {
        ...
        DatabaseManager.shared.userExists(with: email, completion: { [weak self] exists in
            ...
            guard !exists else {
                // User already existe
                strongSelf.alertUserLoginError(message: "Look like a user for that email account already exits.")
                return
            }
            FirebaseAuth.Auth.auth() .createUser(withEmail: email, password: password, completion:{ authResult, error in
                guard authResult != nil, error == nil else{
                    print("Error creatinf user")
                    return
                }
